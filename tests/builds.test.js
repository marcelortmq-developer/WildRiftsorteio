...
test('Base boots excluded from item slots; only category Botas used for boot slot',()=>{assert.ok(items.find(i=>i.name==='Botas da Velocidade').is_boot);assert.ok(!itemPool(items).some(i=>i.name==='Botas da Velocidade'));});
...
test('Intermediate and base items are excluded even with another eligible category',()=>{const mixed={...items[0],id:'synthetic',is_boot:false,categories:['Físico','Defesa','Tier Médio']};for(const c of ['Todos',...BUILD_CATEGORIES])assert.ok(itemPool([...items,mixed],c).every(i=>!i.categories.some(t=>['Tier Médio','Tier Base'].includes(t))));for(const c of ['Tier Médio','Tier Base']){assert.throws(()=>drawBuild(items,c));assert.ok(searchItems(items,c,'').length>0);}});
