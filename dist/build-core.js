...
export const BUILD_CATEGORIES=['Físico','Mágico','Defesa','Suporte','Tier Médio','Tier Base'];
export const ITEM_CATEGORIES=['Todos',...BUILD_CATEGORIES,'Botas','Lutador','Assassino','Atirador'];
...
export function itemPool(items,category='Todos'){
 ...
 return [...new Map(items.filter(i=>i.active!==false&&!i.is_boot&&!i.categories.includes('Botas')&&(category==='Todos'?i.categories.some(c=>BUILD_CATEGORIES.includes(c)):i.categories.includes(category))).map(i=>[i.id,i])).values()];
}
